...
class AitStockLocationResPartner(models.Model):
    ...
    dedicated_customer_location = fields.Boolean(
        string="Dedicated Customer Location", default=False
    )

    dedicated_vendor_location = fields.Boolean(
        string="Dedicated Vendor Location", default=False
    )

    dedicated_subcontracting_location = fields.Boolean(
        string="Dedicated Subcontracting Location", default=False
    )

    dedicated_rental_location = fields.Boolean(
        string="Dedicated Rental Location", default=False
    )

    dedicated_customer_location_created = fields.Boolean(
        string="Dedicated Customer Location Created", default=False
    )

    dedicated_vendor_location_created = fields.Boolean(
        string="Dedicated Vendor Location Created", default=False
    )

    dedicated_subcontracting_location_created = fields.Boolean(
        string="Dedicated Subcontracting Location Created", default=False
    )

    dedicated_rental_location_created = fields.Boolean(
        string="Dedicated Rental Location Created", default=False
    )

    property_stock_rental = fields.Many2one(
        "stock.location",
        string="Ubicazione noleggio",
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if (
                vals.get(DEDICATED_CUSTOMER_LOCATION)
                and vals.get(DEDICATED_CUSTOMER_LOCATION) == True
            ):
            ...
            if (
                vals.get(DEDICATED_VENDOR_LOCATION)
                and vals.get(DEDICATED_VENDOR_LOCATION) == True
            ):
            ...
            if (
                vals.get(DEDICATED_SUB_LOCATION)
                and vals.get(DEDICATED_SUB_LOCATION) == True
            ):
            ...
            if (
                vals.get(DEDICATED_RENTAL_LOCATION)
                and vals.get(DEDICATED_RENTAL_LOCATION) == True
            ):
                # Create the location using _create_location method with 'subcontracting_parent' as the location type
        ...
        for record in res:
            if record.dedicated_subcontracting_location:
                ...
                sub_vendor_location = self.env["stock.location"].search(
                    [("location_id", "=", parent_id), ("name", "=", record.name)]
                )
    ...
    def write(self, vals):
        ...
        for record in self:
            if (
                "dedicated_customer_location" in vals
                and record.dedicated_customer_location
            ):
            ...
            if (
                "dedicated_subcontracting_location" in vals
                and record.dedicated_subcontracting_location
            ):
    ...
    def dedicated_customer_location_checked(self):
        if self.dedicated_customer_location:
            ...
            parent_id = self._check_dedicated_location("customer_parent")
            customer_location = self.env["stock.location"].search(
                [("location_id", "=", parent_id), ("name", "=", self.name)]
            )
            if customer_location:
    ...
    def _check_dedicated_location(self, dedicated_location):
        ...
        parent_id = (
            var.get(dedicated_location) if var.get(dedicated_location) else False
        )
    ...
    def _create_location(self, vals, parent):
        ...
        warehouse = location_env.sudo().search(
            [(NAME, "=", vals.get(NAME)), (LOCATION_ID, "=", parent_id)]
        )
